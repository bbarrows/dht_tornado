...
dht.DHTTree.MAX_LIST_LENGTH = 2

def sb(arr):
    return ''.join(chr(x) for x in arr)

def new_node(arr):
	return dht.DHTPeer(sb(arr), ("127.0.0.1", 30))

bit_iter = dht.string_bit_iterator('\xff\xff\xff\xff\xff\xff\xff\xff')
for b in bit_iter:
    assert(b == 1)
...
count = 0
last = 0
bit_iter = dht.string_bit_iterator('\xaa')
for b in bit_iter:
    assert(b != last)
    last = b
    count = count + 1
...
#assert(dht.byte_array_to_uint([0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA]) == 0xAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA)

p_id =  sb([0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA,0xAA])
dht_inst = dht.DHT(1,[], node_id = p_id)
t = dht.DHTTree(dht_inst)
dht.DHTTree.MAX_LIST_LENGTH = 2
...
node_list = dht.NodeListHeap(p_id)
...
assert(dht.dht_dist("06587bb7abe4fa1f9ef274af74cc9b17eeebf686".decode("hex"), "\x00" * 20) > dht.dht_dist("01587bb7abe4fa1f9ef274af74cc9b17eeebf686".decode("hex"), "\x00" * 20))

assert(dht.dht_dist("06587bb7abe4fa1f9ef274af74cc9b17eeebf686".decode("hex"), "\x00" * 20) > dht.dht_dist("053099bf0ae1b4d1a30107a4f375fbbfb87beda4".decode("hex"), "\x00" * 20))
